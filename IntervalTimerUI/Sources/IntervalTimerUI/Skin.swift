...
public enum Skin: Sendable {
    ...
    /// どちらの地の上でも読めることが要る。
    public var goTint: Color? { self == .normal ? Color(hex: PaletteHex.goInk) : nil }
    public var stopTint: Color? { self == .normal ? Color(hex: PaletteHex.stopInk) : nil }

    /// 増減ボタン。押せる場所だと分かればよいので、控えめに。
    public var stepTint: Color { self == .normal ? Color(hex: PaletteHex.goInk) : ink }
}
